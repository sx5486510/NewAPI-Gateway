...
系统不再使用手工 `priority`/`weight` 控制流量。当前分流由动态评分和健康状态决定：

- 结合价格、余额、最近消耗计算 `value_score` 动态放大/缩小占比；
- 可选开启健康调节，优先尝试当前整点小时失败更少的路由；
- 健康值相同的候选再按贡献值做加权随机不放回。

因此最终分流比例不是固定静态权重，而是由价格、余额、最近消耗和实时失败情况共同决定。
