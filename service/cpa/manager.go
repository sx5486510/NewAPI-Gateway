...
// Compatibility wrappers keep the existing startup and basic CPA controller
// working until the process-wide Runtime is installed by the routing task.
type legacyLifecycleHooks struct {
	mu      sync.RWMutex
	onReady func(string, string)
}

func (h *legacyLifecycleHooks) setReady(callback func(string, string)) {
	h.mu.Lock()
	h.onReady = callback
	h.mu.Unlock()
}

func (h *legacyLifecycleHooks) OnCPAReady(baseURL, apiKey string) {
	h.mu.RLock()
	callback := h.onReady
	h.mu.RUnlock()
	if callback != nil {
...
func (*legacyLifecycleHooks) OnCPAUnavailable() {}
func (*legacyLifecycleHooks) ScheduleCPASync()  {}

var (
	legacyManagerMu    sync.Mutex
	legacyManager      *Manager
	legacyManagerHooks *legacyLifecycleHooks
)

func getLegacyManager(onReady func(string, string)) (*Manager, error) {
	legacyManagerMu.Lock()
	defer legacyManagerMu.Unlock()
	if legacyManager == nil {
		...
		legacyManagerHooks = &legacyLifecycleHooks{}
		legacyManager = NewManager(store, legacyManagerHooks)
	}
	legacyManagerHooks.setReady(onReady)
	return legacyManager, nil
}

func StartFromDB(onReady func(baseURL, apiKey string)) error {
	manager, err := getLegacyManager(onReady)
	if err != nil {
		return err
	}
	return manager.StartFromDB(context.Background())
}

func Stop() {
	legacyManagerMu.Lock()
	manager := legacyManager
	legacyManagerMu.Unlock()
	if manager != nil {
...
func Reload(onReady func(baseURL, apiKey string)) error {
	...
	return manager.Restart(context.Background())
}
...
func IsRunning() bool {
	...
	manager := legacyManager
	legacyManagerMu.Unlock()
	return manager != nil && manager.Status().State == StateRunning
}
