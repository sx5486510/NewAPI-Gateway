...
build-be:
	@mkdir -p ./bin
	go build -ldflags "-s -w -X 'NewAPI-Gateway/common.Version=$$(cat VERSION)+$$(git rev-parse --short HEAD 2>/dev/null || echo nogit)' -X 'NewAPI-Gateway/common.BuildTime=$$(TZ='Asia/Shanghai' date +%Y-%m-%dT%H:%M:%S+08:00)'" -o $(BINARY)
